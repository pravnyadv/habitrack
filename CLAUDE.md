...
src/
  ...
  components/
    ...
    Overview.jsx         # Preact: TWO sections, one per kind, each with its own cards, chip row and
                         ...
                         # habits, quits included) (SSR habits via props). Needs a `today` prop:
                         # module-level TODAY is the epoch during SSR (see Dates, below).
...
- **Dates.** The Worker runs in UTC, so a week strip rendered server-side would be a day off for anyone whose local date differs right then, and would visibly jump when the client re-rendered. The server uses `todayInZone(Astro.locals.runtime.cf.timezone)` (Cloudflare's geo-IP zone) and falls back to `iso(new Date())`. `stats`/`streakStats`/`startOf`/`windowStart`/`heatmapBlock`/`githubGraph`/`streakGraph`/`quitAggregate`/`buildExportRecords` therefore all take an optional `today` argument instead of closing over the module-level `TODAY`. Note `Intl` with `timeZone: undefined` does **not** throw, it silently uses the runtime zone, so `todayInZone` guards the empty case explicitly or the fallback becomes unreachable.
- **Server-side `TODAY` is 1970-01-01, not just "a day off".** On workerd `Date.now()` returns 0 while a module's top level is evaluated, so `export const TODAY = iso(new Date())` in `compute.js` freezes at the epoch during SSR. It is correct in the browser and useless on the server. This is worse than a timezone slip: `windowStart()` becomes 1969-02-01 and every `d <= TODAY` filter drops the whole dataset, so the Overview server-rendered a 1969 grid with a zero in every card and only corrected itself on hydration (shipped that way until `fix/overview-ssr-today`). The trap is the **default parameter**: `today = TODAY` means a call site that forgets to pass it silently gets the epoch instead of failing, which is how this survived unnoticed. Every `.astro` page rendering `<Overview>` passes `today`, and the island prefers the browser's own `TODAY` once mounted. Verify SSR date work with `npm run preview` (real workerd), never `npm run dev`: under Vite the module evaluates in Node, where `Date.now()` is normal and the bug is invisible.
...
Fixtures are built relative to `compute.js`'s own `TODAY`, so the suite is deterministic on any day and in any timezone. Five suites are worth keeping alive as the app changes:
...
- **Injected `today`** (`compute.test.js`) renders the heatmap helpers with `'1970-01-01'`, which is literally what the server passes if a call site forgets the argument, and asserts each one honours it (window bounds, captions, grid width, blanked future cells). Assert on the caption as a **number**: `/0 clean days/` also matches "10 clean days", which let two of these pass a mutation that broke them.
